...
/*   Updated: 2020/04/12 15:39:09 by florianhame      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	cy_roots(t_vec tab[3], t_vec cam_ray, t_cy *cy, t_cam *cam)
{
	...
	a = dot_p(cross_p(cam_ray, tab[0]), cross_p(cam_ray, tab[0]));
	b = 2 * (dot_p(cross_p(cam_ray, tab[0]), cross_p(tab[1], tab[0])) -
	dot_p(cross_p(cam_ray, tab[0]), cross_p(tab[2], tab[0])));
	...
	return (cy_best_root(t, cy, cam, cam_ray));
}
...
double	cy_best_root(t_root t, t_cy *cy, t_cam *cam, t_vec cam_ray)
{
	...
	if (t.t1 > 1)
	{
		p.x = cam->x + (t.t1 * cam_ray.x);
		p.y = cam->y + (t.t1 * cam_ray.y);
		p.z = cam->z + (t.t1 * cam_ray.z);
	...
	if (t.t2 > 1)
	{
		p.x = cam->x + (t.t2 * cam_ray.x);
		p.y = cam->y + (t.t2 * cam_ray.y);
		p.z = cam->z + (t.t2 * cam_ray.z);
		po_sqr = pow(p.x - cy->x, 2) + pow(p.y - cy->y, 2) + pow(p.z - cy->z, 2);
		if (sqrt(fmax(0, po_sqr - pow(cy->diam / 2, 2))) <= cy->height / 2)
			return (t.t2);
	}
