...
/*   Updated: 2020/04/26 16:48:37 by florianhame      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		color_obj(t_data *data, t_obj obj)
{
	...
	while (lgt != NULL)
	{
		...
		if (!lgt_intersection(obj, lgt, data))
			ratio += fmax(0, f_ratio(obj, lgt)) * lgt->power;
		lgt = lgt->next;
	}
	if (obj.id == 4)
		return (color_pl(data, obj, ratio));
	if (obj.id == 5)
		return (color_sp(data, obj, ratio));
	if (obj.id == 6)
		return (color_sq(data, obj, ratio));
	if (obj.id == 7)
		return (color_cy(data, obj, ratio));
	if (obj.id == 8)
		return (color_tr(data, obj, ratio));
	return (back_color);
}
